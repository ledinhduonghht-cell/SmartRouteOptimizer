...
let chargingStationsLayer = null;
let showChargingStations = false;
...
async function loadVehicleData() {
    try {
        ...
        chargingStationsData = data.chargingStations || [];
        ...
        console.log('Đã tải dữ liệu trạm sạc:', chargingStationsData.length, 'trạm sạc');
    } catch (error) {
        console.error('Lỗi khi tải dữ liệu xe:', error);
        // Fallback data
        vehicleData = [
            { id: 'truck_light', name: 'Xe tải nhẹ (1.5 - 5 tấn)', emissionFactor: 0.12, fuelConsumption: 0.25, maxSpeed: 60, fuelType: 'diesel' }
        ];
...
function updateVehicleInfo() {
    ...
    if (selectedVehicle) {
        ...
        // Kiểm tra và hiển thị thông tin sạc nếu là xe điện
        if (selectedVehicle.fuelType === 'electric') {
            chargingSection.style.display = 'block';
            // Tự động hiển thị trạm sạc sau 1 giây
            setTimeout(() => {
                if (!showChargingStations) {
                    document.getElementById('showChargingStations').click();
                }
            }, 1000);
        } else {
            chargingSection.style.display = 'none';
            // Ẩn trạm sạc nếu đang hiển thị
            if (showChargingStations) {
                hideChargingStations();
                document.getElementById('showChargingStations').classList.remove('active');
...
function setupEventListeners() {
    ...
    document.getElementById('showChargingStations').addEventListener('click', toggleChargingStations);
...
async function calculateOptimalRoute() {
    ...
    try {
        ...
        // Nếu là xe điện, tính toán nhu cầu sạc
        if (vehicle && vehicle.fuelType === 'electric') {
            await calculateChargingNeeds(currentRoute);
        }
...
function calculateFuelCost(distance, vehicle, trafficMultiplier, routeType) {
    ...
    // Nếu là xe điện, tính theo kWh
    if (vehicle.fuelType === 'electric') {
        const electricityPrice = 3500; // VNĐ/kWh
        const consumptionPerKm = (vehicle.batteryCapacity || 50) / (vehicle.range || 200);
        const energyNeeded = distance * consumptionPerKm;
        const baseCost = energyNeeded * electricityPrice;
...
function updateStatistics(distance, co2, duration, vehicle, routeType) {
    ...
    if (vehicle.fuelType === 'electric') {
        const consumptionPerKm = (vehicle.batteryCapacity || 50) / (vehicle.range || 200);
        fuelConsumption = (distance * consumptionPerKm).toFixed(1);
    } else {
        fuelConsumption = (distance * vehicle.fuelConsumption).toFixed(1);
    }
    ...
    document.getElementById('fuelConsumption').textContent = `${optimizedFuelConsumption || '0'} ${vehicle.fuelType === 'electric' ? 'kWh' : 'L'}`;
...
async function recalculateRoute(routeType) {
    ...
    try {
        ...
        // Nếu là xe điện, tính toán lại nhu cầu sạc
        if (currentRoute.vehicle.fuelType === 'electric') {
            await calculateChargingNeeds(currentRoute);
        }
...
// Hiển thị/ẩn trạm sạc
async function toggleChargingStations() {
    const btn = document.getElementById('showChargingStations');
    const chargingSection = document.getElementById('chargingSection');
    ...
    if (!showChargingStations) {
        ...
        try {
            ...
            // Lấy trạm sạc từ API
            const stations = await MockAPI.findChargingStations(
                center.lat, 
                center.lng,
                10000,
                vehicle
            ...
            updateChargingStationsList(stations);
            
            showNotification('Đã hiển thị trạm sạc điện', 'success');
        } catch (error) {
            console.error('Lỗi khi tải trạm sạc:', error);
        ...
        showChargingStations = false;
        showNotification('Đã ẩn trạm sạc điện', 'info');
    }
}
...
function showChargingStationsOnMap(stations) {
    // Xóa layer cũ nếu có
    if (chargingStationsLayer) {
        map.removeLayer(chargingStationsLayer);
    }
    
    chargingStationsLayer = L.layerGroup();
    
    stations.forEach(station => {
        // Tạo marker cho trạm sạc
        const marker = L.marker(station.coordinates, {
            icon: L.divIcon({
                className: 'charging-marker',
                html: `<i class="fas fa-charging-station" style="color: ${
                    station.type === 'ultra_fast' ? '#e74c3c' : 
                    station.type === 'fast' ? '#3498db' : 
                    station.type === 'medium' ? '#2ecc71' : '#7f8c8d'
                }; font-size: 22px;"></i>`,
                iconSize: [30, 30]
            })
        }).addTo(chargingStationsLayer);
        
        // Thêm popup
        marker.bindPopup(`
            <div class="station-popup">
                <h4><i class="fas fa-charging-station"></i> ${station.name}</h4>
                <div class="details-grid">
                    <div class="label">Địa chỉ:</div>
                    <div class="value">${station.address}</div>
                    
                    <div class="label">Loại sạc:</div>
                    <div class="value">${station.type === 'ultra_fast' ? 'Siêu nhanh' : 
                                      station.type === 'fast' ? 'Nhanh' : 
                                      station.type === 'medium' ? 'Trung bình' : 'Chậm'}</div>
                    
                    <div class="label">Công suất:</div>
                    <div class="value">${station.chargeSpeed}</div>
                    
                    <div class="label">Kết nối:</div>
                    <div class="value">${station.connectorType}</div>
                    
                    <div class="label">Trạng thái:</div>
                    <div class="value" style="color: #4CAF50;">
                        <i class="fas fa-plug"></i> ${station.availablePlugs} trạm trống
                    </div>
                    
                    <div class="label">Giá:</div>
                    <div class="value">${station.pricePerKwh.toLocaleString('vi-VN')} VNĐ/kWh</div>
                    
                    <div class="label">Giờ mở cửa:</div>
                    <div class="value">${station.operatingHours}</div>
                </div>
                <div class="actions">
                    <button class="btn-sm" onclick="setAsWaypoint(${station.coordinates[0]}, ${station.coordinates[1]}, '${station.name}')">
                        <i class="fas fa-map-marker-alt"></i> Điểm dừng
                    </button>
                    <button class="btn-sm" onclick="calculateRouteToStation(${station.coordinates[0]}, ${station.coordinates[1]}, '${station.name}')">
                        <i class="fas fa-route"></i> Đến trạm
                    </button>
                </div>
            </div>
        `);
    ...
    chargingStationsLayer.addTo(map);
}

// Ẩn trạm sạc
function hideChargingStations() {
    if (chargingStationsLayer) {
        map.removeLayer(chargingStationsLayer);
        chargingStationsLayer = null;
    }
}

// Cập nhật danh sách trạm sạc trong sidebar
function updateChargingStationsList(stations) {
    const container = document.getElementById('chargingStationsList');
    
    if (!stations || stations.length === 0) {
        container.innerHTML = `
            <div style="text-align: center; padding: 20px; color: #7f8c8d;">
                <i class="fas fa-charging-station fa-2x" style="margin-bottom: 10px;"></i>
                <p>Không tìm thấy trạm sạc trong khu vực</p>
            </div>
        `;
        return;
    }
        ...
        <div class="station-item" data-id="${station.id}">
            <div class="station-header">
                ...
                <span class="station-type">
                    ${station.type === 'ultra_fast' ? 'SIÊU NHANH' : 
                      station.type === 'fast' ? 'NHANH' : 
                      station.type === 'medium' ? 'TRUNG BÌNH' : 'CHẬM'}
                </span>
            </div>
            ...
            <div class="station-details">
                ...
                ${station.chargeSpeed} • ${station.chargingTime}
            </div>
            <div class="station-details">
            ...
            <div class="station-availability">
                <span class="available">
                    <i class="fas fa-check-circle"></i> ${station.availablePlugs} trạm trống
                </span>
                <span class="distance">
                    <i class="fas fa-road"></i> ${station.distance} km
                </span>
            </div>
        </div>
    `).join('');
    ...
    // Thêm sự kiện click cho từng trạm
    container.querySelectorAll('.station-item').forEach((item, index) => {
        item.addEventListener('click', () => {
            const station = stations[index];
            map.setView(station.coordinates, 15);
            
            // Mở popup cho marker tương ứng
            const markers = chargingStationsLayer.getLayers();
            if (markers[index]) {
...
// Tính toán nhu cầu sạc
async function calculateChargingNeeds(route) {
    const vehicleTypeId = document.getElementById('vehicleType').value;
    const vehicle = vehicleData.find(v => v.id === vehicleTypeId);
    ...
    try {
        const distance = route.data.distance / 1000; // km
        const chargingNeeds = await MockAPI.calculateChargingNeeds(distance, vehicle);
        
        // Cập nhật giao diện
        document.getElementById('energyNeeded').textContent = `${chargingNeeds.energyNeeded} kWh`;
        document.getElementById('chargingTime').textContent = `${(chargingNeeds.estimatedChargingTime * 60).toFixed(0)} phút`;
        document.getElementById('chargingCost').textContent = `${parseInt(chargingNeeds.estimatedCost).toLocaleString('vi-VN')} VNĐ`;
        
        // Hiển thị cảnh báo nếu cần sạc
        if (chargingNeeds.needsCharging) {
            showNotification(`Cần sạc thêm ${chargingNeeds.batteryNeededPercent}% pin cho chuyến đi`, 'warning');
        }
        
        // Lấy trạm sạc dọc tuyến đường
        const centerLat = (route.start[0] + route.end[0]) / 2;
        const centerLng = (route.start[1] + route.end[1]) / 2;
        const nearbyStations = await MockAPI.findChargingStations(centerLat, centerLng, 5000, vehicle);
        
        // Lấy đề xuất lịch trình sạc
        const schedule = await MockAPI.optimizeChargingSchedule(
            route.data,
            vehicle,
            nearbyStations
        );
        
        // Thêm đề xuất vào AI Suggestions
        const aiContainer = document.getElementById('aiSuggestions');
        const currentContent = aiContainer.innerHTML;
        
        const chargingSuggestionsHTML = `
            <p><strong><i class="fas fa-charging-station"></i> Đề xuất sạc điện:</strong></p>
            ${schedule.suggestions.map(s => `<p><i class="fas fa-bolt" style="color: #FFD700;"></i> ${s}</p>`).join('')}
            ${schedule.recommendedStations.length > 0 ? `
                <p><strong>Trạm sạc đề xuất:</strong></p>
                ${schedule.recommendedStations.map(s => `
                    <p><i class="fas fa-map-marker-alt" style="color: #4CAF50;"></i> ${s.name} (${s.distance}km)</p>
                `).join('')}
            ` : ''}
            <p style="margin-top: 10px; color: #2ecc71;">
                <i class="fas fa-coins"></i> Ước tính tiết kiệm: ${schedule.estimatedCostSavings}% chi phí sạc
            </p>
        `;
        
        aiContainer.innerHTML = currentContent + chargingSuggestionsHTML;
        
    } catch (error) {
        console.error('Lỗi tính toán nhu cầu sạc:', error);
    }
}

// Thiết lập trạm sạc làm điểm dừng
function setAsWaypoint(lat, lng, name) {
    if (!window.startCoords || !window.endCoords) {
        alert('Vui lòng thiết lập điểm xuất phát và điểm đến trước');
        return;
    }
    
    // Hỏi người dùng muốn thêm làm điểm dừng ở đâu
    const position = prompt(`Thêm "${name}" làm điểm dừng:\n\n1 - Sau điểm xuất phát\n2 - Trước điểm đến\n\nNhập 1 hoặc 2`);
    
    if (position === '1') {
        // Tạm thời lưu điểm dừng
        window.waypoint = [lat, lng];
        showNotification(`Đã thêm "${name}" làm điểm dừng sau khi xuất phát`, 'success');
    } else if (position === '2') {
        // Có thể cần tính toán lại route với điểm dừng
        showNotification(`Đã thêm "${name}" làm điểm dừng trước khi đến đích`, 'success');
    }
}

// Tính toán route đến trạm sạc
function calculateRouteToStation(lat, lng, name) {
    if (!window.startCoords) {
        alert('Vui lòng thiết lập điểm xuất phát trước');
        return;
    }
    
    const confirmRoute = confirm(`Tính toán tuyến đường từ vị trí hiện tại đến "${name}"?`);
    
    if (confirmRoute) {
        // Thiết lập điểm đến là trạm sạc
        setAsEndPoint(lat, lng, name);
        
        // Tính toán route
        setTimeout(() => {
            document.getElementById('calculateRoute').click();
        }, 500);
    }
}

// =========================
// KẾT THÚC CHỨC NĂNG TRẠM SẠC
// =========================
...
function clearRoute() {
    ...
    // Reset thông tin sạc
    document.getElementById('energyNeeded').textContent = '-- kWh';
    document.getElementById('chargingTime').textContent = '-- phút';
    document.getElementById('chargingCost').textContent = '-- VNĐ';
    document.getElementById('chargingStationsList').innerHTML = '';
...
window.setAsWaypoint = setAsWaypoint;
window.calculateRouteToStation = calculateRouteToStation;
